...
AddEventHandler('fetchjob', function()
  ...
  AddEventHandler('drugs:sell', function()
	...
	local moonshine = xPlayer.getInventoryItem('moonshine_pooch').count
	local paymentc = math.random (500,1000)
	local paymentw = math.random (350,800)
	local paymentm = math.random (350,800)
	local paymento = math.random (350,800)
		...
		if coke >= 1 and success == true then
  		...
  		elseif weed >= 1 and success == true then
			...
			xPlayer.removeInventoryItem('opium_pooch', 1)
  			...
  			selling = false
			elseif moonshine >= 1 and success == true then
  				TriggerClientEvent("pNotify:SetQueueMax", source, "lmao", 5)
				TriggerClientEvent("pNotify:SendNotification", source, {
					text = "You have sold a gallon of Moonshine for $" .. paymento ,
					type = "success",
					progressBar = false,
					queue = "lmao",
					timeout = 2000,
			...
			elseif selling == true and success == false and notintrested == true then
  			...
  			selling = false
  			elseif meth < 1 and coke < 1 and weed < 1 and opium < 1 and moonshine < 1 then
				TriggerClientEvent("pNotify:SetQueueMax", source, "lmao", 5)
				TriggerClientEvent("pNotify:SendNotification", source, {
...
AddEventHandler('checkD', function()
	...
	local opium = xPlayer.getInventoryItem('opium_pooch').count
	local moonshine = xPlayer.getInventoryItem('moonshine_pooch').count
	if meth >= 1 or coke >= 1 or weed >= 1 or opium >= 1 or moonshine >= 1 then
